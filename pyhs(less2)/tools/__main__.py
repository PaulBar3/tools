...
try:
    ...
    from tools import zip_archive
    from tools import copy_files_meta, change_name_if_exists
except:
...
def run_task(task, matches):
    ...
    cwd: str = os.getcwd()
    path_for_copy: str = os.path.join(cwd, "copied")
    ...
    if task == "1":
        ...
        archive_name: str = "backup-" + time.strftime("%Y-%m-%d_%H-%M-%S")
        dst: str = os.path.join(cwd, archive_name)
        target_dir = copy_files_meta(path_for_copy, matches)
        path: str = zip_archive(dst, target_dir, delete_target=True)
        print(f"Архив: {path}")
...
def get_task():
    commands = ("0", "1", "2")
    while True:
        command: str = input(f"Копировать - 1, Архивировать - 2, Отмена - 0\n>> ")
...
def get_search_params():
    start_path: str = input("Введите путь (пустой ввод - текущий cwd)\n>> ") or os.getcwd()
    ...
    time_stamp: int = int(input(prompt) or 0) * 3600
